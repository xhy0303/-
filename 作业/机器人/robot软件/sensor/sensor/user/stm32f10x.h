...
typedef struct
{
  __IO uint32_t CRL;  // ÅäÖÃ¼Ä´æÆ÷µÍ32Î»£ºconfiguration register low (GPIOx_CRL) (x=A..E)
  __IO uint32_t CRH;  // ÅäÖÃ¼Ä´æÆ÷¸ß32Î»£ºconfiguration register high(GPIOx_CRH) (x=A..E)
  __IO uint32_t IDR;  // ÊäÈëÊý¾Ý¼Ä´æÆ÷£ºinput data register (GPIOx_IDR) (x=A..E)
  __IO uint32_t ODR;  // Êä³öÊý¾Ý¼Ä´æÆ÷£ºoutput data register (GPIOx_ODR) (x=A..E)
  __IO uint32_t BSRR; // Î»ÖÃÎ»/¸´Î»¼Ä´æÆ÷£ºbit set/reset register (GPIOx_BSRR) (x=A..E)
  __IO uint32_t BRR;  // Î»¸´Î»¼Ä´æÆ÷£ºbit reset register (GPIOx_BRR) (x=A..E)
  __IO uint32_t LCKR; // Î»Ëø¶¨¼Ä´æÆ÷£ºlock register (GPIOx_LCKR) (x=A..E)
} GPIO_TypeDef;
...
typedef struct
{
  __IO uint32_t CR;//Ê±ÖÓ¿ØÖÆ¼Ä´æÆ÷£ºClock control register
  __IO uint32_t CFGR;//Ê±ÖÓÅäÖÃ¼Ä´æÆ÷£ºClock configuration register
  __IO uint32_t CIR;//Ê±ÖÓÖÐ¶Ï¼Ä´æÆ÷£ºClock interrupt register
  __IO uint32_t APB2RSTR;//APB2ÍâÉè¸´Î»¼Ä´æÆ÷£ºAPB2 Peripheral reset register
  __IO uint32_t APB1RSTR;// APB1ÍâÉè¸´Î»¼Ä´æÆ÷£ºAPB1 Peripheral reset register
  __IO uint32_t AHBENR;//AHBÍâÉèÊ±ÖÓÊ¹ÄÜ¼Ä´æÆ÷£ºAHB Peripheral Clock enable register
  __IO uint32_t APB2ENR;//APB2ÍâÉèÊ±ÖÓÊ¹ÄÜ¼Ä´æÆ÷£ºAPB2 Peripheral Clock enable register
  __IO uint32_t APB1ENR;//APB1ÍâÉèÊ±ÖÓÊ¹ÄÜ¼Ä´æÆ÷£ºAPB1 Peripheral Clock enable register
  __IO uint32_t BDCR;//±¸·ÝÓò¿ØÖÆ¼Ä´æÆ÷£ºBackup domain control register
  __IO uint32_t CSR;//¿ØÖÆ/×´Ì¬¼Ä´æÆ÷£ºControl/status register
